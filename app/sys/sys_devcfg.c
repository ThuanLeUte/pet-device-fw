...
/* Private defines ---------------------------------------------------------- */
#define BLUFI_NAME_FMT_EMPTY       "PET-DEVICE-%02x%02x"
#define BLUFI_NAME_FMT_AND_ID      "PET-DEVICE-%s"
...
void sys_devcfg_init(void)
{
  ...
  if (FLAG_QRCODE_SET_SUCCESS != g_nvs_setting_data.dev.qr_code_flag)
  {
    ...
    ESP_LOGI(TAG, "BT MAC: " ESP_BD_ADDR_STR, ESP_BD_ADDR_HEX(esp_bt_dev_get_address()));
...
static void m_blufi_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
  ...
  case ESP_BLUFI_EVENT_INIT_FINISH:
  {
    ...
    // NOTE: QRcode format: CM1002022030001, Manufature data display: 1002022030001
    if (FLAG_QRCODE_SET_SUCCESS == g_nvs_setting_data.dev.qr_code_flag)
    {
